...
For `csh` / `tcsh`, shell integration works with startup scripting, token exchange, prompt-status updates, and cwd tracking. Command pre-execution hooks are shell-limited in `csh`, so command-start tracking is best-effort there. `tcsh` gets richer completion behavior than plain `csh`.
