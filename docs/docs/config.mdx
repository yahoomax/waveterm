...
| term:pasteonmiddleclick              | bool     | set to false to disable pasting the clipboard into the terminal on middle mouse click (defaults to true)                                                                                                                                                       |
| term:touchscroll                     | bool     | set to false to disable touch screen scrolling in terminal blocks (defaults to true)                                                                                                                                                                          |
| term:touchtextselect                 | bool     | set to false to disable touch screen text selection in terminal blocks (defaults to true). When enabled, long-press selects a word (like the browser block); drag to extend the selection. Selection handles appear at the start and end for adjusting the range |
...
| editor:touchtextselect               | bool     | set to false to disable touch screen text selection in the code editor (defaults to true). When enabled, drag horizontally to select text; selection handles appear at the start and end for adjusting the range. Vertical swipes still scroll. Long-press continues to open the editor context menu |
| editor:copyonselect                  | bool     | set to true to enable editor copy-on-select (defaults to false)                                                                                                                                                                                               |
| editor:extendedsyntax                | bool     | set to false to disable extended syntax highlighting for shell-like and other script files in the code editor (defaults to true). When enabled, files such as `.csh`, `.tcsh`, `.ps1`, `.pl`, and `.tcl` use Monaco highlighting (csh/tcsh use shell highlighting) |
...
```json
{
    ...
    "editor:touchtextselect": true,
    "editor:copyonselect": false,
    "editor:extendedsyntax": true,
    ...
    "term:pasteonmiddleclick": true,
    "term:touchscroll": true,
    "term:touchtextselect": true,
    "term:durable": false,
